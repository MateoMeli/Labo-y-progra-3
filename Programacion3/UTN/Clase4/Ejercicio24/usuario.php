   ...
   class Usuario{
    ...
    public static function LeerDeJson($archivo){
        ...
        while(!feof($archivo)){
            ...
            $usuario = json_decode($renglon, true);
    ...
    public static function ImprimirArrayJson($array){
        $retorno = "<ul>";
        foreach($array as $key => $value){
            $retorno .= '<li>Id: ' . $value["_id"]. ' Nombre: ' . $value["_nombre"] . ' Clave: ' . $value["_clave"] . ' Mail: ' . $value["_mail"] . 
            '<br><img src="'.$value["_foto"] .'" /></li>';
        }
        $retorno .= "<ul>";
        return $retorno;
    }
